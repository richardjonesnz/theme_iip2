 ...
 * modified to add text to the header area
 * based on a theme setting by Richard Jones http://richardnz.net
...
class core_renderer extends \core_renderer {
    ...
    public function full_header() {
        ...
        // This div holds any custom text from the theme settings for IIP2
        // If there is text in the page setting headerarea use it
        //  - provided we are on the front page
        $header_text = $PAGE->theme->settings->headerarea;
        if ( (!empty($header_text)) && ($PAGE->pagetype == 'site-index') ){
            $html .= html_writer::start_div('header-text');
            $html .= html_writer::nonempty_tag('h2',$PAGE->theme->settings->headerarea);  
            $html .= html_writer::end_div();
        }
   ...
   public function context_header($headerinfo = null, $headinglevel = 1) {
        ...
        if ($this->should_display_main_logo($headinglevel)) {
            //  If there is text in the page setting headerarea use it
            $header_text = $PAGE->theme->settings->headerarea;
            // Otherwise use the site full name for alt-text
            if (empty($header_text)) {
                $header_alt = format_string($SITE->fullname, true, array('context' => context_course::instance(SITEID)));
            } else {
                $header_alt = $header_text;
            }
            $header_content = html_writer::empty_tag('img', [
                'src' => $this->get_logo_url(null, 150), 'alt' => $header_alt]);               
